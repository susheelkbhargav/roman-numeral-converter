...
public class RomanNumeralConverterService {
	...
	private void checkSupportedRange(int numberToConvert) {
		...
		if (numberToConvert < AppConstants.MIN_NUMBER_TO_CONVERT || numberToConvert > AppConstants.MAX_NUMBER_TO_CONVERT) {
			throw new RangeViolationException(
					"The number entered must be between " + AppConstants.MIN_NUMBER_TO_CONVERT + "and " + AppConstants.MAX_NUMBER_TO_CONVERT);
		}
	}
